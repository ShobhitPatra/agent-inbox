...
it("renders the agent header, the live transcript and the focused approval", () => {
  const { lastFrame, unmount } = render(
    <AgentDetail state={seeded()} agentId="coder" cursor={0} steerText={null} armed={false} />,
  );
...
it("shows the steer composer when a steer buffer is active", () => {
  const { lastFrame, unmount } = render(
    <AgentDetail state={seeded()} agentId="coder" cursor={0} steerText="focus on login" armed={false} />,
  );
  const frame = lastFrame() ?? "";
...
it("shows the armed-cancel banner", () => {
  const { lastFrame, unmount } = render(
    <AgentDetail state={seeded()} agentId="coder" cursor={0} steerText={null} armed={true} />,
  );
  expect(lastFrame() ?? "").toContain("press c again to cancel coder");
  unmount();
});
...
it("renders a no-pending notice when the agent has no pending approvals", () => {
  ...
  const { lastFrame, unmount } = render(
    <AgentDetail state={s} agentId="coder" cursor={0} steerText={null} armed={false} />,
  );
